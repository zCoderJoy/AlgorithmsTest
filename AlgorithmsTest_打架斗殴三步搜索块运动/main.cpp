...
using namespace std;
//全局参数
const int AmplitudeBinNumbs=40;//幅度直方图列个数
const int AngleBinNumbs=60;//角度直方图列个数
...
int bins_angle_pre[500]={0};//存储之前角度直方图数据
int bins_amplitude_pre[500]={0};//存储之前幅度直方图数据
...
void DrawSubHistogram(IplImage *dst,int binsNums,int binsCur[],int binsPre[]);//画相减直方图
bool isFighting(vector<double> vectorAmplitude,vector<double> vectorAngle,int binsAmpl[],int binsAngl[]);//是否是打架斗殴,区域矢量个数大于30
//主函数
int main()
{
	//9X9初始化
	...
	IplImage* imgA = cvLoadImage("D:/VS2012/data/Videos/2014-07-16Fighting/Channel2CutPictures/120.bmp",CV_LOAD_IMAGE_GRAYSCALE);//加载图像
	...
	IplImage* histimg_Angle_sub = cvCreateImage( cvSize(320,200), 8, 3 );//角度直方图统计
	IplImage* histimg_Amplitude_sub = cvCreateImage( cvSize(320,200), 8, 3 );//角度直方图统计
	...
	//处理多帧图像
	for (int ii=120;ii<4030;ii++)////###################调用多幅图片进行测试##########################$$$$$$$$$$$$$$$
	{
		//cout<<"正在处理第 "<< i<<"帧；\n";
		string filename="";
		char tmp[20];
		...
		cvShowImage("AmplitudeImage",histimg_Amplitude);//展示幅度直方图
		cvShowImage("AngleImage",histimg_Angle);
		cvShowImage("幅度直方图之差",histimg_Amplitude_sub);
		cvShowImage("角度直方图之差",histimg_Angle_sub);
...
void DrawSubHistogram(IplImage *dst,int binsNums,int binsCur[],int binsPre[])//画相减直方图
{
	...
	for( int i = 0; i < binsNums; i++ )
	{
		double val = ( (binsCur[i]-binsPre[i])*dst->height/50.0 );//得到一维矩阵第i的高度
		CvScalar color = CV_RGB(255,255,0); //(hsv2rgb(i*180.f/hdims);
		cvRectangle( dst, cvPoint(i*bins_w,dst->height/2),
			cvPoint((i+1)*bins_w,(int)(dst->height/2 - val)),//在目标图像上画矩形，左上到右下
			color, 1, 8, 0 );
	}	
...
bool isFighting(vector<double> vectorAmplitude,vector<double> vectorAngle,int binsAmpl[],int binsAngl[])//是否是打架斗殴,区域矢量个数大于30
{
	...
	averageAmpl=sumAmplitude/Nums;//求得幅度平局值
	averageAngl=sumAngle/Nums;//求角度平均值
	//求直方图角度的方差
